...
## OVERVIEW
Read-only CLI path for inspecting known LiteDB collections. It is an admin/debug utility, not part of the background host pipeline.
...
| Collection whitelist | `../../TrendReporter2.Core/Persistence/TrendCollectionNames.cs` | `TrendCollectionNames.All` only. |

## CONVENTIONS
- `data-view <collection> [--limit <n>] [--json] [--config <path>]` is parsed in `Program.cs`.
- Collections are validated against `TrendCollectionNames.All` at parse/read time.
...
- Do not fetch, enrich, match, score, push, or schedule from this path.
