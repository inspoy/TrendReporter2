...
| `SummarySource` | `string?` | `TitleOnly` / `Tavily` / `LLM` |
| `NeedEnrichment` | `bool` | 是否需要调用 Tavily |
...
| `LastPushAvgRank` | `double?` | 最近一次推送时平均排名 |
...
```json
{
  ...
  "stage": "Escalating",
  "progressSummary": "先由少数信源报道，随后迅速扩散并升温，目前仍在持续发酵。",
  "reason": "string",
  "summary": "string"
}
```
...
- `Cluster` 模型：做事件归并
- `Judge` 模型：做重要性修正、摘要补足和发展进程润色

实现建议：

- 两类模型独立配置
- 使用 OpenAI 兼容接口
- 全部要求 `response_format=json` 或等价约束
...
只要满足任一条件，即标记 `NeedEnrichment = true`。

### 12.2 候选事件召回

V1 不引入向量库，采用轻量召回：

1. 选取最近 `historyHours` 内 `Active` 事件
2. 再补充最近 7 天内与当前标题关键词重合的 `Stale` 事件
3. 基于标题和摘要做粗匹配，保留最多 20 个候选
...
3. 若返回 `matched = true` 且 `confidence >= mergeThreshold`，归并到已有事件
4. 否则创建新事件
...
- `UniqueSourceCount >= sourceCount && AvgRank <= rankThreshold`
- 最近 `trendThreshold` 小时热度分整体上升
- 事件属于陈旧复活事件
...
1. 取最近 `trendThreshold` 小时的热度样本
2. 拆成前半窗和后半窗
3. 若后半窗平均热度高于前半窗，认定整体升温
4. 差值越大，趋势分越高

示例公式：

```text
trendScore = clamp((avgRecent - avgPast) / max(avgPast, 0.1), 0, 1)
```
...
- 当前平均排名相较上次推送改善 `>= rankImproveThreshold`
...
4. 调用 `Judge` 模型将结构化节点润色为自然语言进程摘要
5. 若 LLM 失败，则回退为模板化摘要
