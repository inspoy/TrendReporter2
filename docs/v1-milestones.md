...
| 进度 | 里程碑 | 名称 | 目标 |
| --- | --- | --- | --- |
| ✅ | `V1M0` | 项目骨架与基础设施 | 建立可运行工程骨架、配置、日志、数据库初始化 |
| ✅ | `V1M1` | 新闻抓取与原始入库 | 打通 NewsNow 抓取与 `content_item/content_snapshot` 落库 |
| ✅ | `V1M2` | 正文增强与摘要补充 | 接入 Tavily，补足新闻摘要 |
| ✅ | `V1M3` | 事件建模与归并 | 建立事件表、候选召回、LLM 归并 |
| ✅ | `V1M4` | 评分与即时推送 | 实现热度分、重要性判定、重复推送控制 |
| ✅ | `V1M5` | 定时摘要与降噪 | 实现早晚摘要、黑名单与摘要去重 |
| ✅ | `V1M6` | 稳定性与回归测试 | 完成日志、错误处理、测试和运行校验 |
...
#### `V1M1-T5` 实现 `content_snapshot` 落库

- 为每轮抓取的每条新闻生成快照
- 保存 `RunId`、`CapturedAt`、`Rank`

#### `V1M1-T6` 实现 `fetch_run` 记录
...
#### `V1M2-T3` 抽象并实现 `ITavilyClient`

- 根据 Tavily API 文档封装请求
- 处理成功响应
- 处理超时、限流、失败响应

#### `V1M2-T4` 实现 `EnrichmentService`
...
#### `V1M3-T4` 抽象 `IClusterLlmClient`

- 接 OpenAI 兼容接口
- 约束返回结构化 JSON
- 处理非法 JSON 和空返回

#### `V1M3-T5` 实现 `EventMatcher`
...
#### `V1M4-T9` 实现 Unipush 推送器

- 按文档生成请求
- 填充 `cate = default`
- 发送即时推送

#### `V1M4-T10` 记录 `push_log`
...
- `V1M1` 产出的抓取数据是后续所有模块的输入
- `V1M2` 的摘要质量会直接影响 `V1M3` 归并效果
- `V1M4` 依赖 `V1M3` 的事件结构
- `V1M5` 依赖 `V1M4` 的评分结果
- `V1M6` 适合在主功能打通后统一补齐
...
| 并行组 | 可并行任务 |
| --- | --- |
| `P1` | `V1M0-T3` 配置模型、`V1M0-T6` LiteDB 初始化、`V1M0-T7` 调度器骨架 |
| `P2` | `V1M1-T2` NewsNowClient、`V1M1-T4` content_item 仓储、`V1M1-T6` fetch_run |
| `P3` | `V1M2-T2` 增强判定、`V1M2-T3` TavilyClient |
| `P4` | `V1M3-T3` 候选召回、`V1M3-T4` Cluster LLM Client |
| `P5` | `V1M4-T2` 热度分、`V1M4-T5` Judge LLM Client、`V1M4-T8` Unipush |
| `P6` | `V1M5-T1` 黑名单过滤、`V1M5-T3` 摘要消息组装 |
...
- 完成 `V1M0`
- 完成 `V1M1`
- 在 `V1M2` 中先只实现“标题即摘要”的降级版本，不接 Tavily
- 在 `V1M3` 中先用规则召回 + LLM 归并
- 在 `V1M4` 中先实现首次推送，不做重复推送
- 暂缓 `V1M5` 摘要
