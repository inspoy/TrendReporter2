...
> 自动从传统媒体榜单中发现值得关注的重要事件，并持续追踪其热度变化、后续进展和发展过程。
...
### 3.5 事件发展进程（Event Progress）

事件发展进程用于回答“这件事是怎么一路发展到现在的”。

对用户来说，除了知道事件当前热不热，还需要知道：

- 事件最早何时进入监控
- 后续是否快速扩散到更多信源
- 当前属于“初现、发酵、升级、后续、回落”中的哪个阶段
- 最近一次值得关注的进展是什么
...
系统在配置的时间点生成“事件列表”摘要，只输出当前周期内最值得关注的前 N 个事件，并为每个事件附带简要的发展进程说明。
